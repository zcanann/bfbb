...
#define NGC_SOUND_S32_TO_F64_BIAS BINK_NGC_S32_TO_F64_BIAS
...
const f64 BINK_NGC_S32_TO_F64_BIAS = 4503601774854144.0;
...
static s32 Lock(BINKSND PTR4* snd, u8 PTR4* PTR4* addr, u32 PTR4* len)
{
    ...
    if (NGC_SOUND_STATE(snd)->lock_index >= 0) {
        ...
        right_task = &tasks[NGC_SOUND_STATE(snd)->lock_index + NGC_SOUND_RIGHT_TASK_OFFSET];
